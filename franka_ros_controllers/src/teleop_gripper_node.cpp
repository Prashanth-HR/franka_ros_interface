...
#include <franka_ros_controllers/teleop_gripper_paramConfig.h>
...
class TeleopGripperClient {
 public:
  ...
  bool init(const std::shared_ptr<ros::NodeHandle>& pnh) {
    ...
    dynamic_server_teleop_gripper_param_ = std::make_unique<
        dynamic_reconfigure::Server<franka_ros_controllers::teleop_gripper_paramConfig>>(
        dynamic_reconfigure_teleop_gripper_param_node_);
 ...
 private:
  ...
  void teleopGripperParamCallback(
      const franka_ros_controllers::teleop_gripper_paramConfig& config,
      uint32_t /*level*/) {
